...
function HomePage() {
  ...
  return <>
    <section className="hero-layout"><div className="hero-top"><article className="greeting-card"><div><p>대전경찰청장</p><h2>열린청장실</h2><span>항상 열린 마음으로 대전시민의 의견을 귀담아 듣겠습니다.</span></div></article><div className="related-sites">{Object.entries(relatedSites).map(([label, sites]) => <div className="site-picker" key={label}><span>{label}</span><details><summary>{label}</summary><ul>{sites.map(site => <li key={site}><button type="button">{site}</button></li>)}</ul></details></div>)}</div></div><div className="hero-bottom"><section className="hero" style={{ backgroundImage: `linear-gradient(90deg, rgba(8,30,83,.72), rgba(8,30,83,.12)), url(${imageUrl(slides[slide])})` }}><div className="hero-content"><p>시민과 함께하는</p><h1>안전한 대전,<br />든든한 대전경찰</h1><div><button onClick={() => setSlide((slide + slides.length - 1) % slides.length)} aria-label="이전 슬라이드">←</button><button onClick={() => setSlide((slide + 1) % slides.length)} aria-label="다음 슬라이드">→</button></div></div></section><div className="quick-menu-panel">{quickServices.map((label, index) => <button key={label}><strong>0{index + 1}</strong><span>{label}</span></button>)}</div></div></section>
